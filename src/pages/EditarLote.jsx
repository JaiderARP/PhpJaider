...
const EditarLote = () => {
    ...
    const handleEditUser1 = async () => {
        try {
            await Api.put(`/Actualizar/${id}`,Lote);
            navigate("/listar")
        } catch (error) {
    ...
    const handleEditUser2 = async () => {
        try {
            await Api.patch(`/Desactivar/${id}`,Lote);
            navigate("/listar")
        } catch (error) {
    ...
    return (
        <div>
            ...
            <div className="max-w-xs">
                <input className="shadow appearance-none border rounded w-full py-2 m-3 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="text" placeholder="fecha_creacion" value={Lote.fecha_creacion} onChange={(e)=> setLotes({ ...Lote, fecha_creacion: e.target.value})}/>
                ...
                <input className="shadow appearance-none border rounded w-full py-2 m-3 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="number" placeholder="latitud" value={Lote.latitud} onChange={(e)=> setLotes({ ...Lote, latitud: e.target.value})}/>
